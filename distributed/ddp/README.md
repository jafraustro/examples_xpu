...
values needed to create a `ProcessGroup` is to use the `torchrun` script
provided with PyTorch (`torchrun` is the recommended
way to launch distributed jobs, replacing the `launch.py` script).

You can check the documentation here [torchrun](https://docs.pytorch.org/docs/stable/elastic/run.html)

When the DDP application is started via `torchrun`, `torchrun` sets up necessary environment variables (like `RANK`, `WORLD_SIZE`, `MASTER_ADDR`, `MASTER_PORT`) for each process. Your script should read these environment variables to initialize the distributed process group (e.g., using `torch.distributed.init_process_group`).
To use `torchrun`, an application needs to adhere to the following convention:
...
`torchrun` supports single-node distributed training by spawning one or more processes per node. It works for both CPU and GPU training. For GPU training, each distributed process is assigned to a single GPU.
...
The application can be launched via `torchrun` as follows on an 8 GPU node with one process per GPU:

```sh
torchrun --nproc_per_node=8 example.py --local_world_size=8
```
